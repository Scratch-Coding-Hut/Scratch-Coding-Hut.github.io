...
app.post("/forum", async (req, res) => {
    if (!isAdmin(req)) return res.status(403).json({ error: "You must be an admin to create topics" });
...
app.delete("/forum/:topicName", async (req, res) => {
    if (!isAdmin(req)) return res.status(403).json({ error: "You must be an admin to delete topics" });
...
app.delete("/forum/:topicName/reply/:replyIndex", async (req, res) => {
    if (!isAuthenticated(req)) return res.status(401).json({ error: "You must be logged in to delete comments" });
...
app.get("/profile", (req, res) => {
    if (!isAuthenticated(req)) return res.status(401).json({ error: "You must be logged in to view your profile" });
